<script lang="ts">
	...
	import { wbsService } from "$lib/services/wbs";
	...
	interface Props {
		...
		holidays?: Holiday[];
		baselineSets?: BaselineSet[];
		restDays?: number[];
		onScroll?: (e: Event) => void;
		viewMode?: "Day" | "Week" | "Month" | "Quarter";
		hoveredNodeId?: number | null;
		onHover?: (id: number | null) => void;
		onProgressChange?: (node: WBSNode, progress: number) => void;
		onDatesChange?: (node: WBSNode, newStart: Date, newEnd: Date) => void;
		onCreateDependency?: (fromId: number, toId: number) => void;
		onDeleteDependency?: (dep: WBSDependency) => void;
		scrollTop?: number;
	}
	...
	let dragProgress = $state<Record<number, number>>({});

	// Drag-bar (move dates) state
	let dragBarNodeId = $state<number | null>(null);
	let dragBarOffset = $state<Record<number, number>>({});

	// Resize state
	let resizingNodeId = $state<number | null>(null);
	let resizingType = $state<"left" | "right" | null>(null);
	let resizingOffset = $state<Record<number, number>>({});

	// Connect-mode state
	let connectingFromId = $state<number | null>(null);
	...
	function getTimelineDates(mode: string, nodesList: WBSNode[]) {
		...
		if (nodesList && nodesList.length > 0) {
			...
			nodesList.forEach((n) => {
				if (n.planned_start_date) {
					const s = normalizeToLocalMidnight(
						n.planned_start_date,
					).getTime();
					if (s < minTime) minTime = s;
				}
				if (n.planned_end_date) {
					const e = normalizeToLocalMidnight(
						n.planned_end_date,
					).getTime();
	...
	let timelineDates = $derived(getTimelineDates(viewMode, nodes));
	...
	function getBarPosition(node: WBSNode) {
		...
		const start = normalizeToLocalMidnight(node.planned_start_date);
		const end = normalizeToLocalMidnight(node.planned_end_date);

		let pixelOffset = getPixelOffset(start);
		let pixelWidth = getPixelOffset(end) - pixelOffset;

		if (resizingNodeId === node.id) {
			const offset = resizingOffset[node.id] || 0;
	...
	function getBaselinePosition(baselineNode: WBSBaselineNode) {
		if (
		...
		const start = normalizeToLocalMidnight(baselineNode.planned_start_date);
		const end = normalizeToLocalMidnight(baselineNode.planned_end_date);
	...
	let contentHeight = $derived(nodes.length * ROW_HEIGHT);
	...
	let barPositions = $derived.by(() => {
		const map: Record<number, { x1: number; x2: number; y: number }> = {};
	...
	function getDisplayProgress(node: WBSNode): number {
		if (draggingNodeId === node.id && dragProgress[node.id] !== undefined) {
			return dragProgress[node.id];
		}
	...
	function startDragBar(e: MouseEvent, node: WBSNode) {
		...
		e.preventDefault();
		dragBarNodeId = node.id;
		dragBarOffset[node.id] = 0;
		const startX = e.clientX;
		const initialStart = new Date(node.planned_start_date!);
		const initialEnd = new Date(node.planned_end_date!);

		function onMove(me: MouseEvent) {
			dragBarOffset = {
				...dragBarOffset,
				[node.id]: me.clientX - startX,
			};
		}

		function onUp() {
			const dx = dragBarOffset[node.id] ?? 0;
			dragBarNodeId = null;
			...
			if (dx !== 0 && Math.abs(dx) > 2) {
				...
				onDatesChange?.(node, newStart, newEnd);
			}
			dragBarOffset = { ...dragBarOffset, [node.id]: 0 };
		}
	...
	// --- Resize bar ---
	function startResize(e: MouseEvent, node: WBSNode, side: "left" | "right") {
		if (!hasPermission($authStore.user, $authStore.token, "project:update"))
			return;
		e.stopPropagation();
		e.preventDefault();
		...
		function onUp() {
			const dx = resizingOffset[node.id] ?? 0;
			resizingNodeId = null;
			resizingType = null;
			document.removeEventListener("mousemove", onMove);
			document.removeEventListener("mouseup", onUp);
			...
			resizingOffset = { ...resizingOffset, [node.id]: 0 };
		}
	...
	function startDrag(e: MouseEvent, node: WBSNode, barWidth: number) {
		...
		dragProgress[node.id] = node.progress;
		...
		function onMove(me: MouseEvent) {
			...
			dragProgress = { ...dragProgress, [node.id]: newProgress };
		}
		function onUp() {
			const finalProgress = dragProgress[node.id] ?? node.progress;
			draggingNodeId = null;
			document.removeEventListener("mousemove", onMove);
			document.removeEventListener("mouseup", onUp);
			document.body.style.cursor = "";
			if (finalProgress !== node.progress) {
				onProgressChange?.(node, finalProgress);
			}
		}
		document.body.style.cursor = "ew-resize";
		document.addEventListener("mousemove", onMove);
			...
			if (targetBar) {
				...
				if (targetId && targetId !== connectingFromId) {
					const exists = dependencies.some(
						(d) =>
							d.predecessor_id === connectingFromId &&
							d.successor_id === targetId,
					);
					if (!exists) {
	...
	// Draw arrow path: from right edge of predecessor to left edge of successor
	function arrowPath(
	...
	let visibleRange = $derived.by(() => {
		...
		const endIdx = Math.min(
			nodes.length,
			Math.ceil((scrollTop + ganttBodyClientHeight) / ROW_HEIGHT) +
				BUFFER,
		);
	...
	function isArrowVisible(pred: { y: number }, succ: { y: number }) {
		const minY = Math.min(pred.y, succ.y);
		const maxY = Math.max(pred.y, succ.y);
		const viewBottom = scrollTop + ganttBodyClientHeight;
		const margin = ROW_HEIGHT * 2;
		return maxY >= scrollTop - margin && minY <= viewBottom + margin;
	}
</script>
		...
		<div class="relative w-fit min-w-full">
			...
			>
				...
				{#each dependencies as dep}
					{@const pred = barPositions[dep.predecessor_id]}
					{@const succ = barPositions[dep.successor_id]}
					{#if pred && succ && isArrowVisible(pred, succ)}
						<path
							class="dep-line cursor-pointer transition-all duration-300 pointer-events-auto"
			...
			<div
				...
				style="width: {svgWidth}px; transform: translateY({offsetY}px);"
			>
				{#each visibleNodes as node, idx (node.id)}
					{@const globalIdx = visibleRange.start + idx}
					{@const pos = getBarPosition(node)}
					{@const displayProgress = getDisplayProgress(node)}
					{@const baselineNode = baselineSets[0]?.nodes.find(
						(b) => b.node_id === node.id,
					)}
					...
					<div
						...
						style="width: {svgWidth}px; background-color: {hoveredNodeId ===
						node.id
							? 'rgba(19, 55, 236, 0.08)'
							: 'transparent'}"
					...
					>
						{#if (draggingNodeId === node.id || resizingNodeId === node.id || dragBarNodeId === node.id) && pos.visible}
						...
						{#if pos.visible}
							{@const itemHolidays = getHolidaysInRange(
								...
								normalizeToLocalMidnight(node.planned_end_date),
							)}
							<div
								role="button"
								tabindex="0"
								aria-label="Gantt Bar for {node.title}"
							...
							hover:cursor-grab outline-none focus:ring-2 focus:ring-primary/40"
								...
								title="{node.title} ({displayProgress}%)"
								onmousedown={(e) => startDragBar(e, node)}
								onkeydown={(e) => {
							...
							>
								...
								<!-- Resize Handles -->
								{#if node.type !== "Milestone" && connectingFromId === null}
									<!-- Left Resize Handle -->
									<div
										role="button"
										tabindex="0"
								...
								{#if connectingFromId === null}
									<div
										...
										class="drag-handle absolute top-0 bottom-0 flex items-center justify-center cursor-ew-resize z-40 group/drag {draggingNodeId ===
										node.id
