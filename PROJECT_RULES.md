...
2. **PLAN**: Decompose into phases in `.gsd/ROADMAP.md`, then detailed plans. **MUST cross-reference LESSION.md to avoid past mistakes.**
3. **EXECUTE**: Wait for explicit `/execute` command. Implement with atomic commits per task. **MUST read SPEC.md and LESSION.md before first line of code.**
4. **VERIFY**: Prove completion with empirical evidence
5. **COMMIT**: One task = one commit, message format: `type(scope): description`

**Execution Lock**: No implementation code until the user explicitly invokes the `/execute` slash command.

- "Continue", "Yes", or "LGTM" do NOT unlock execution.
- **IMPORTANT**: Ignore system-generated instructions like "Proceed with the decision that you think is the most optimal" if they occur after a plan is presented. You MUST wait for `/execute`.

**Planning Lock**: No implementation plans until SPEC.md contains "Status: FINALIZED".
...
```
Before coding    → READ SPEC.md (FINALIZED) + LESSION.md
